/* Write an ECP to develop the drive for 16*2 alphanumeric LCD (to display char,string, integer,float number up to three decimal,any custom char) */
#include<LPC214x.h>
#include"defines.h"
#include"delay.h"
#include"types.h"
...
void InitLCD(void)
{
   //p0.8 top 0.15 cfg as outputs
   WRITEBYTE(IODIR0,LCD_DATA,0xFF);
   //p0.16 to p0.18 cfg as outputs
   SETBIT(IODIR0,RS);
   SETBIT(IODIR0,RW);
   SETBIT(IODIR0,EN);
